import os
from transformers import Trainer, TrainingArguments, TrainerCallback, DistilBertForSequenceClassification, DistilBertTokenizerFast
import torch
...
def compute_metrics(eval_pred):
    ...
    log_file_name = f"test_mia_attack_results.log"
...
def load_model_from_epoch(epoch, model_type="distilbert-base-uncased", base_model_type="distilbert-base-uncased"):
...
def train_model(train_dataset, eval_dataset, tokenizer, training_args, device, model_type="distilbert-base-uncased"):
    with tempfile.TemporaryDirectory() as temp_dir:
        model = DistilBertForSequenceClassification.from_pretrained(
            "distilbert-base-uncased", 
            num_labels=num_labels, 
    ...
    plot_gradient_norms(gradient_capture_callback.gradient_stats, ['attention'], model_type="standard")
...
def dp_train_model(train_dataset, eval_dataset, tokenizer, training_args, privacy_args, device, model_type="dp-distilbert-base-uncased"):
    with tempfile.TemporaryDirectory() as temp_dir:
        model = DistilBertForSequenceClassification.from_pretrained(
            "distilbert-base-uncased", 
            num_labels=num_labels, 
    ...
    # Freeze the positional embedding layer
    ...
    plot_dp_gradient_norms(gradient_capture_callback.gradient_stats, privacy_args.target_epsilon, ["attention"], model_type="standard")