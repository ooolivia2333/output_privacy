...
from transformers import set_seed, DistilBertTokenizerFast, TrainingArguments, BertTokenizerFast
from config import device, seed, standard_num_epochs, standard_train_batch_size, standard_eval_batch_size, standard_lr
...
from training import train_model, compute_metrics, SaveModelCallback, load_model_from_epoch, dp_train_model
...
train_model(datasets["train"], datasets["test"], tokenizer, training_args, device, model_type="distilbert-base-uncased")
...
for epoch in [1.0, 3.0]:
    model = load_model_from_epoch(epoch, model_type="bert-base-uncased")
    ...
    with open(log_file_name, "a") as log_file:
        log_file.write("standard model non-dp trained with not modified dataset\n")
    ...
    perform_mia_and_log_results(model, epoch, train_logits, train_labels, train_losses, test_logits, test_labels, test_losses, train_indices, test_indices, datasets, model_type="bert-base-uncased")

# # Train model with LoRA on modified dataset
# print("Training model with LoRA on modified dataset")
train_model(modified_train_dataset, modified_test_dataset, tokenizer, training_args, device, model_type="QNLI-distilbert-base-uncased_modified")
...
for epoch in [1.0, 3.0]:
    model = load_model_from_epoch(epoch, model_type="QNLI-distilbert-base-uncased_modified")
    ...
    log_file_name = "model_performance.log"
    with open(log_file_name, "a") as log_file:
        log_file.write("standard model not dp trained with modified dataset")
        log_file.write(f"Accuracy: {accuracy}\n")
        log_file.write(f"Precision: {precision}\n")
        log_file.write(f"Recall: {precision}\n")
        log_file.write(f"f1: {f1}\n")
    ...
    perform_mia_and_log_results(model, epoch, train_logits, train_labels, train_losses, test_logits, test_labels, test_losses, train_indices, test_indices, datasets, model_type="QNLI-distilbert-base-uncased-modified")
...
for epsilon in [4.0]:
    ...
    dp_train_model(datasets["train"], datasets["test"], tokenizer, training_args, privacy_args, device, model_type=f'dp-distilbert-base-uncased-epsilon-{epsilon}')
    
    for epoch in [1.0, 3.0]:
        model = load_model_from_epoch(epoch, model_type=f'dp-distilbert-base-uncased-epsilon-{epsilon}')
        accuracy, precision, recall, f1 = evaluate(model, test_loader)
        ...
        log_file_name = "model_performance.log"
        with open(log_file_name, "a") as log_file:
            log_file.write("standard model dp trained with not modified dataset\n")
            log_file.write(f"Accuracy: {accuracy}\n")
            log_file.write(f"Precision: {precision}\n")
            log_file.write(f"Recall: {precision}\n")
            log_file.write(f"f1: {f1}\n")
        ...
        train_logits, train_labels, train_losses, _ = collect_logits_labels_losses(train_loader, model, torch.nn.CrossEntropyLoss(reduction="none"), train_indices, device)
        test_logits, test_labels, test_losses, _ = collect_logits_labels_losses(test_loader, model, torch.nn.CrossEntropyLoss(reduction="none"), test_indices, device)
        perform_mia_and_log_results(model, epoch, train_logits, train_labels, train_losses, test_logits, test_labels, test_losses, train_indices, test_indices, datasets, model_type=f'dp-distilbert-base-uncased-epsilon-{epsilon}')
...
for epsilon in [4.0]:
    ...
    dp_train_model(modified_train_dataset, modified_test_dataset, tokenizer, training_args, privacy_args, device, model_type=f'QNLI-dp-distilbert-base-uncased-epsilon-{epsilon}-modified')
    
    for epoch in [1.0, 3.0]:
        model = load_model_from_epoch(epoch, model_type=f'QNLI-dp-distilbert-base-uncased-epsilon-{epsilon}-modified')
        accuracy, precision, recall, f1 = evaluate(model, modified_test_loader)
        ...
        log_file_name = "model_performance.log"
        with open(log_file_name, "a") as log_file:
            log_file.write("standard model dp trained with modified dataset")
            log_file.write(f"Accuracy: {accuracy}\n")
            log_file.write(f"Precision: {precision}\n")
            log_file.write(f"Recall: {precision}\n")
            log_file.write(f"f1: {f1}\n")
        ...
        train_logits, train_labels, train_losses, _ = collect_logits_labels_losses(modified_train_loader, model, torch.nn.CrossEntropyLoss(reduction="none"), train_indices, device)
        test_logits, test_labels, test_losses, _ = collect_logits_labels_losses(modified_test_loader, model, torch.nn.CrossEntropyLoss(reduction="none"), test_indices, device)
        perform_mia_and_log_results(model, epoch, train_logits, train_labels, train_losses, test_logits, test_labels, test_losses, train_indices, test_indices, datasets, model_type=f'QNLI-dp-distilbert-base-uncased-epsilon-{epsilon}-modified')